...
if( $wp_query->is_posts_page ) {
	...
	$layout = sigma_set( $meta, 'layout', 'right' );
	$sidebar = sigma_set( $meta, 'sidebar', 'default-sidebar' );
	$title = sigma_set($meta1, 'header_title');
} else {
	$settings  = _WSH()->option(); 
	if(sigma_set($_GET, 'layout_style')) $layout = sigma_set($_GET, 'layout_style'); else
	$layout = sigma_set( $settings, 'archive_page_layout', 'right' );
	$sidebar = sigma_set( $settings, 'archive_page_sidebar', 'default-sidebar' );
	$title = sigma_set($settings, 'archive_page_header_title');
}
$layout = sigma_set( $_GET, 'layout' ) ? sigma_set( $_GET, 'layout' ) : $layout;
    ...
    <div class="container">
        <div class="row">
            <div class="col-sm-12">
                ...
                <h3><?php if($title) echo wp_kses_post($title); else esc_html_e('Our Blog', 'sigma'); ?></h3>
