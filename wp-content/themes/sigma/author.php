<?php sigma_bunch_global_variable(); 
	...
	$layout = sigma_set( $settings, 'author_page_layout', 'full' );
	$sidebar = sigma_set( $settings, 'author_page_sidebar', 'default-sidebar' );
	$view = sigma_set( $settings, 'author_page_view', 'list' );
	_WSH()->page_settings = array('layout'=>$layout, 'sidebar'=>$sidebar);
	$layout = ($layout) ? $layout : 'full';
	$sidebar = ($sidebar) ? $sidebar : 'default-sidebar';
    if (is_active_sidebar( $sidebar )) {$layout = 'right';} else{$layout = 'full';}
	$classes = ( !$layout || $layout == 'full' || sigma_set($_GET, 'layout_style')=='full' ) ? 'col-sm-12' : 'col-sm-9';
	$title = sigma_set($settings, 'author_page_header_title');
?>
